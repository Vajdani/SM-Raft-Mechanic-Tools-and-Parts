function raft_LoadCraftingRecipes( recipePaths )
	...
	if not g_raft_craftingRecipes then
		g_raft_craftingRecipes = {}
		for name, path in pairs( recipePaths ) do
			...
			g_raft_craftingRecipes[name] = { path = path, recipes = recipes, recipesByIndex = recipesByIndex }
		end
	end

	-- Preload refinery recipes
	if not g_raft_refineryRecipes then
		g_raft_refineryRecipes = sm.json.open( "$CONTENT_DATA/CraftingRecipes/refinery.json" )
		for _,recipe in pairs( g_raft_refineryRecipes ) do
