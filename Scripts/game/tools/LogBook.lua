...
function LogBook.sv_n_requestInitData( self, _, player )
	...
	--RAFT
	local world = clientData.locations["crashedShip"].world
	clientData.locations["rangerstation"] = { world = world, pos = sm.vec3.new(-1728, -1656, 41)}
	clientData.locations["radio quest"] = { world = world, pos = sm.vec3.new(-1820.5, 167.5, -7)}
	clientData.locations["trader"] = { world = world, pos = sm.vec3.new(1536, 2048, 20)}
	clientData.locations["veggies"] = { world = world, pos = sm.vec3.new(-768, 1280, 20)}
	clientData.locations["wocTemple"] = { world = world, pos = sm.vec3.new(3108.874511718750, 2785.419433593750, 9.309272766113281)}
	clientData.locations["fruits"] = { world = world, pos = sm.vec3.new(0, -1344, 20)}
	clientData.locations["scrapcity"] = { world = world, pos = sm.vec3.new(1600.25, -2064.75, 55)}
	--RAFT
...
function LogBook.cl_onLogItemClicked( self, _, _, log )
	...
	if log and log.uuid then
		...
		self.cl.gui:setVisible( "WaypointButton", self.cl.log.icon ~= "" ) --RAFT
	end
